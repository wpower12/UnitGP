...
static final Logger logger = LogManager.getLogger( UnitGP.class.getName() );
...
private void select(){
	...
	for( int c = 0; c < numCrossover/2; c++ ) {
		...
		p1.applyMutation( probMutation );
		p2.applyMutation( probMutation );
...
/**
 * Sets the weightedFitness to the fitness value.  This lets us sort by actual
 * fitness, not proprotional to the fitness.  This is used when we want the
 * actual fittest member. (For simulation at the end)
 */
